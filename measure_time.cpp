...
void testFunction(ofstream &file, Point c,double r,int m, double R, int n_points, bool (*func)(Point, vector<Point>)){
    ...
    chrono::duration<double, std::micro> time;

    auto start = chrono::high_resolution_clock::now();

    for(int i=0;i<test.size();i++){
            results[i]=func(test[i],polygon);
    }

    auto end = chrono::high_resolution_clock::now();
    auto duration = chrono::duration_cast<chrono::microseconds>(end - start);
    file<<m<<";"<<duration.count()<<endl;
...
int main() {
   ...
   Point c={1,1};       //centroid
    ...
    int m=7;            //number of vertices of the polygon
    double R=5;         //radius of generated points
    int n_points=1000;  //number of generated points
    int n_tests=100;

    file<<"O(N) algorithm"<<endl;
    for(int i=1;i<n_tests+1;i++){
        testFunction(file,c,r,i*10,R,n_points,&testPolygonON);
    }
    file<<"O(LogN) algorithm"<<endl;
    for(int i=1;i<n_tests+1;i++){
        testFunction(file,c,r,i*10,R,n_points,&testPolygonLogN);
    }
